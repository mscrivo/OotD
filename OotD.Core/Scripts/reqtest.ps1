...
# Set the default error action to Continue
...
try {
    ...
    # Use AD credentials to run Get-ADUser with error action set to Continue
    $employee = Get-ADUser -Filter { Description -eq $employeeNumber } -Credential $creds -ErrorAction Continue | Select-Object -ExpandProperty DistinguishedName
