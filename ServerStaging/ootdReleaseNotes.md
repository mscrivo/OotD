...
- Fresh new app icon!
- Updated to latest .net 10.0.9

Fixes:

- Fixed issue where scrolling the calendar was not possible when "Disable Editing" was enabled (#251)
- Fixed issue where the window could not be moved or accessed after disconnecting a monitor/docking station (#219)
- Fixed issue where non-calendar views (Tasks, Notes, etc.) would reset to the default calendar view on restart (#220)
- Fixed crash when renaming an instance from the tray icon menu (#174)

Internal Tooling:

- Started work on integrating SignPath.io for code signing to get rid of installer warnings in the future
